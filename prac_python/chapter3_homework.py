...
def question2():
    Forrest_Gump = db.movies.find_one({'title':'포레스트 검프'})
    
    db.movies.update_many({'released_year':Forrest_Gump['released_year']},{'$set':{'released_year':2025}})
    return

# Q3. 영화의 상영시간이 "펄프 픽션"보다 긴 영화들 중 "석양의 무법자"를 제외한 영화들 모두 삭제해주세요.
def question3():
    Pulp_Fiction = db.movies.find_one({'title':'펄프 픽션'})

    db.movies.delete_many({'running_time':{'$gt':Pulp_Fiction['running_time']},'title':{'$ne':'석양의 무법자'}})
    return
