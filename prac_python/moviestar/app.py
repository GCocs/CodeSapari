...
@app.route('/subPage')
def subPage():
    return render_template('temp_movie.html')
...
def show_movies():
    ...
    # 1. db에서 temp False인 movies 목록을 검색합니다. 주어진 정렬 방식으로 정렬합니다.
    ...
    if sortMode == 'likes':
        movies = list(db.movies.find({'temp': False}, {}). sort('likes', -1))
    elif sortMode == 'running-time':
        movies = list(db.movies.find({'temp': False}, {}). sort('running_time', -1))
    elif sortMode == 'released_year':
        movies = list(db.movies.find({'temp': False}, {}). sort('released_year', -1))
    else:
...
@app.route('/api/temp_list', methods=['GET'])
def show_temp_movies():
    movies = list(db.movies.find({'temp': True}))
    return jsonify({'result': 'success', 'movies_list': movies})

@app.route('/api/favorite', methods=['POST'])
def favorite_movies():
    id = request.form['id']
    db.movies.update_one({'_id': ObjectId(id)}, {'$set': {'temp': False}})
...
def temp_movies():
    id = request.form['id']
    db.movies.update_one({'_id': ObjectId(id)}, {'$set': {'temp': True}})
    return jsonify({'result': 'success'})
